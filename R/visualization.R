...
#' @param pal_setup A dataframe with 2 columns - the \code{RColorBrewer} palette setup to be used, or a string vector of \code{RColorBrewer} palettes, 
#' or a string vector of colors. Default value is \code{'Set2'} in \code{RColorBrewer}.
#' 
#' @return A plot.
...
#' @param n An integer - number of genes to be plotted for each \code{seurat_cluster}.
...
#' It can also be a string vector with three color values, corresponding to the \code{min},\code{median},and \code{max} of the numeric variable. 
#' For a categorical variable, the element can be the names of palettes in \code{RColorBrewer}. The number of palettes can be more than 1, in case there are many levels to color. 
